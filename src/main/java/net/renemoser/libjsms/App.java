...
public class App {
	public static void main(String[] args) {
		if (args.length < 4) {
			System.err.println("Use: java -cp LibJSMS-x.y.jar net.renemoser.libjsms.App <userid> <password> <phoneNumber> <message>");
			System.exit(0);
		}
	    ...
	    try {    	
    	    ...
    	    if (!Sunrise.getShortMessage().isEmpty()) {
    	    	System.out.println("Message '" + 
    	    			Sunrise.getShortMessage() + 
    	    			"' was sent to " + 
    	    			Sunrise.getPhoneNumber());
    	    }
    	} catch(Exception e) {
    	    e.printStackTrace();
    	}
